...
public interface ExamRoomRepository extends JpaRepository<ExamRoom, UUID> {
    ...
    Page<ExamRoom> findByCreatedByOrderByCreatedAtDesc(User createdBy, Pageable pageable);

    List<ExamRoom> findByStatusAndStartTimeLessThanEqual(RoomStatus status, LocalDateTime time);

    List<ExamRoom> findByStatusInAndEndTimeLessThanEqual(List<RoomStatus> statuses, LocalDateTime time);
}
